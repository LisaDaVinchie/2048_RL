...
def add_new_tile(grid: np.ndarray) -> np.ndarray:
...
def merge_tiles(line: np.ndarray) -> np.ndarray:
...
def move(grid: np.ndarray, direction):
    ...
    new_grid = np.array([merge_tiles(row) for row in grid])
...
def game_step(grid: np.ndarray, direction: str) -> np.ndarray:
    ...
    if np.array_equal(grid, new_grid):
        return grid
    new_grid = add_new_tile(new_grid)
    return new_grid